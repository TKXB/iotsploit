...
class Monitor(BaseMonitor):
    ...
    def process_case(self, idx: int, payload: bytes, result: HarnessResult) -> None:
        ...
        if result.crashed:
            self.crash_count += 1
            # The CAN reading of a payload belongs to CANMonitor, which already
            # does it. Doing it here too made every non-CAN campaign log a
            # frame-by-frame decode of bytes that are not frames -- ten
            # thousand lines that bury the finding they are attached to.
            logger.warning(
                "[Case %d] CRASH detected, %d bytes: %s", idx, len(payload), payload[:64].hex()
            )
