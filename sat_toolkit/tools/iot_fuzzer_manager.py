...
class IoTFuzzerManager:
    ...
    Follows adapter pattern to integrate with iot_protocol_fuzzer without direct dependency
    """
    ...
    def __init__(self):
        ...
        logger.info("IoT Fuzzer Manager initialized")
    ...
    def pause_campaign(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            campaign_state['paused_at'] = datetime.now().isoformat()
    ...
    def reset_campaign(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            campaign_state['iterations_completed'] = 0
            campaign_state['crashes_found'] = 0
            campaign_state['timeouts_occurred'] = 0
            campaign_state['errors_encountered'] = 0
            campaign_state['status'] = 'reset'
            campaign_state['reset_at'] = datetime.now().isoformat()
    ...
    def get_campaign_status(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Merge Django state with adapter status
            combined_status = {
                **campaign_state,
                **realtime_status,
                'progress_percentage': self._calculate_progress_percentage(campaign_id)
            }
    ...
    def get_campaign_statistics(self, campaign_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add Django-calculated metrics
            campaign_state = self.active_campaigns[campaign_id]
            statistics.update({
                'campaign_duration': self._calculate_campaign_duration(campaign_id),
                'average_iterations_per_second': self._calculate_average_iterations_per_second(campaign_id),
                'crash_rate': self._calculate_crash_rate(campaign_id),
                'timeout_rate': self._calculate_timeout_rate(campaign_id)
            })
    ...
    def _start_campaign_task(self, campaign_id: str) -> None:
        ...
        try:
            ...
            campaign_state = self.active_campaigns[campaign_id]
            campaign_state['task_id'] = f"mock_{campaign_id}"
    ...
    def _calculate_final_statistics(self, campaign_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_iterations': campaign_state['iterations_completed'],
            'crashes_found': campaign_state['crashes_found'],
            'timeouts_occurred': campaign_state['timeouts_occurred'],
            'errors_encountered': campaign_state['errors_encountered'],
            'duration': self._calculate_campaign_duration(campaign_id)
        }
    
    def _calculate_progress_percentage(self, campaign_id: str) -> float:
        """Calculate campaign progress percentage"""
        campaign_state = self.active_campaigns[campaign_id]
        if campaign_state['iterations_total'] == 0:
            return 0.0
        return (campaign_state['iterations_completed'] / campaign_state['iterations_total']) * 100
    
    def _calculate_campaign_duration(self, campaign_id: str) -> float:
    ...
    def _calculate_average_iterations_per_second(self, campaign_id: str) -> float:
        ...
        return campaign_state['iterations_completed'] / duration
    
    def _calculate_crash_rate(self, campaign_id: str) -> float:
        """Calculate crash rate percentage"""
        campaign_state = self.active_campaigns[campaign_id]
        if campaign_state['iterations_completed'] == 0:
            return 0.0
        return (campaign_state['crashes_found'] / campaign_state['iterations_completed']) * 100
    
    def _calculate_timeout_rate(self, campaign_id: str) -> float:
        """Calculate timeout rate percentage"""
        campaign_state = self.active_campaigns[campaign_id]
        if campaign_state['iterations_completed'] == 0:
            return 0.0
        return (campaign_state['timeouts_occurred'] / campaign_state['iterations_completed']) * 100
