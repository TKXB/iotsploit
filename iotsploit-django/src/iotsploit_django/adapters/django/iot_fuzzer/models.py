...
- It is intentionally placed under `iotsploit_django.adapters.django.*` to respect
  Ports & Adapters boundaries (core must not depend on it).
- The app label remains `iotsploit_django` because this module lives under the
  `iotsploit_django` package which is in INSTALLED_APPS, so existing migrations keep working.
"""
