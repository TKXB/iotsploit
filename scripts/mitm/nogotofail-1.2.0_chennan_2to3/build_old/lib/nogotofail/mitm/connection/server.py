...
class Server:
    ...
    def on_select(self, r, w, x):
        # for fd in r + w + x:
        for fd in list(r) + list(w) + list(x):            
        ...
        if (sum(1 for _ in r) == 0 and now - self.last_reap > 600) or now - self.last_reap > 3600:
            self.last_reap = now
    ...
    def setup_connection(self, client_socket):
        ...
        connection = (
            self.connection_class(
                ...
                self.app_blame))
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"start accpet")
        if connection.start():
    ...
    def set_select_fds(self, connection):
        ...
        r, w, x = new_fds
        # # r.append(connection.client_socket)
        # # w.append(connection.client_socket)
        # new_fds = r, w, x
        # connection.select_fds = new_fds
        # self.fds[connection] = new_fds
        self.read_fds.update(r)
        self.write_fds.update(w)
        self.ex_fds.update(x)
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,connection.client_socket)
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,self.read_fds)
        for fd in set(r + w + x):
    ...
    def _remove_fds(self, connection):
        if connection in self.fds:
            ...
            for fd in r:
                # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,self.read_fds)
                # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,fd)
