...
class BaseConnection(object):
    ...
    def _on_server_connected(self):
        ...
        self.server_bridge_fn = self._bridge_server
        self.client_bridge_fn = self._bridge_client
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"set select")
    ...
    def _start_server_connect_nonblocking(self):
        ...
        try:
            ...
            # self.set_select_fds(rlist=[self.client_socket],wlist=[self.server_socket])
            # Try and connect, this will probably raise an EINPROGRESS
            # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"{},{}".format(self.server_addr,self.server_port))
            self.server_socket.connect((self.server_addr, self.server_port))
            # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"{},{}".format(self.server_addr,self.server_port))
            # If we finished instantly call _on_server_connected()
            self._on_server_connected()
        except socket.error as e:
            # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,e.args)
            # We expect an EINPROGRESS from the connect call.
            if e.args[0] == "timed out":
                # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,e)
                self.set_select_fds(rlist=[self.client_socket],wlist=[self.server_socket])
    ...
    def bridge(self, sock):        
        ...
        """
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"newnewsocket")
        self.last_used = time.time()
    ...
    def _bridge_client(self):
        # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,"newsocket")
        try:
            ...
            if not self.ssl:
                ...
                rest = self._check_for_ssl(client_request)
                # self.handler.log(logging.CRITICAL, rest)
                if rest == True:
                    return not self.closed
                elif not self.isTLS:
                    # logging.getLogger("nogotofail.mitm").log(logging.CRITICAL,client_request)
                    # print(client_request)
                    self.handler.log(logging.CRITICAL, "MITM Success! Not TLS!")
                    self.handler.log_event(
                        logging.CRITICAL,
                        connection.AttackEvent(
                            self.handler.connection, self.handler.name, True,
                            "not tls"))
                    self.handler.connection.vuln_notify(util.vuln.VULN_NOT_TLS)
            try:
