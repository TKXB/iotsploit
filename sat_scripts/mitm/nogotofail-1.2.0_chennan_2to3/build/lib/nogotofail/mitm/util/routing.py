...
def run_iptables_cmd(command, ipv4=True, ipv6=False, ignore_failed=True):
    ipv4_bin = "iptables"
    ipv6_bin = "ip6tables"
...
def enable_redirect_rules(port, ipv6=False):
    ...
    try:
        ...
        run_iptables_cmd(
                "-t nat -A PREROUTING -i wlan0 --jump %s" % REDIRECT_CHAIN, ipv6=ipv6)

        run_iptables_cmd(
                "-t nat -A %s -i wlan0 -p tcp -j REDIRECT --to-ports %s" % (REDIRECT_CHAIN, port),
                ipv6=ipv6)
    except Exception:
        logger.exception("Failed to setup routing rules")
        disable_redirect_rules(ipv6=ipv6)
